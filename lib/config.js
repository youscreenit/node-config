 ...
 */
var Config = function() {
  // in case of a symbolic link get the real path
 ...
 * environment variable to "Y" prior to running your application.
 * </p>
 *
...
Config.prototype.watchForConfigFileChanges = function(interval) {
  ...
  if (interval === 0 || process.env['NODE_CONFIG_DISABLE_FILE_WATCH'] == "Y") {
    return;
  }
  ...
  var onFileChange = function(retry) {
    FileSystem.readFile(runtimeJsonFilename, 'UTF-8', function(err, fileContent) {
      ...
      if (err) {
        console.error("Error loading " + runtimeJsonFilename, err);
        return;
      }
  ...
  try {
    ...
    console.error("Error watching for file: " + runtimeJsonFilename, e);
  }
...
Config.prototype._persistConfigsOnChange = function(objectToWatch) {
  ...
  if (process.env['NODE_CONFIG_PERSIST_ON_CHANGE'] === 'N') {
    return;
  }
 ...
 *
 * <p>
 * hostname is the $HOST environment variable if set, otherwise the $HOSTNAME
 ...
 *
 * <p>
 * If the $NODE_APP_INSTANCE environment variable is set, then files with this
 * appended will be loaded.  See the Multiple Applicstion Instances section of
 * the main documentaion page for more information.
 * </p>
 *
 * @protected
...
Config.prototype._loadFileConfigs = function() {
  ...
  // Get the deployment type from NODE_ENV
    var deployment = process.env.NODE_ENV || t._getCmdLineArg('NODE_ENV') || 'development';
  ...
  baseNames.forEach(function(baseName) {
    extNames.forEach(function(extName) {
      ...
      if (appInstance) {
        fullFilename = CONFIG_DIR + '/' + baseName + '-' + appInstance + '.' + extName;
...
Config.prototype.resetRuntime = function(callback) {
    FileSystem.writeFile(runtimeJsonFilename, '{}', function(err, written, buffer) {
        if(err) {
 ...
 * @param searchFor {STRING} Which command line argument you are searching for.
...
Config.prototype._getCmdLineArg = function (searchFor) {
    ...
    for (var argvIt in cmdLineArgs) {
        var match = cmdLineArgs[argvIt].match(new RegExp(searchFor + '[= ]*(.*)$', 'i'));

        if (previouslyMatched) return cmdLineArgs[argvIt];
